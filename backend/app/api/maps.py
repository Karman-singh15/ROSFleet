...
from app.services.pgm_to_png import PgmError, pgm_file_to_png
...
def get_map_image(map_id: int, db: Session = Depends(get_db)):
    """The map as a PNG, so a browser can display it.

    ROS stores maps as PGM, which no browser renders. Converting here rather
    than at upload time means a map edited on disk (or regenerated by
    scripts/generate_sim_map.py) is served fresh with no re-upload.
    """
    record = map_service.get_map(db, map_id)
    if record is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "map not found")
    try:
        png = pgm_file_to_png(record.image_path)
    except FileNotFoundError:
        raise HTTPException(status.HTTP_404_NOT_FOUND,
                            "the map image file is missing from disk")
